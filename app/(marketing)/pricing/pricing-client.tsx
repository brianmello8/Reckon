    ...
    <div className="mt-12">
      ...
      <div className="flex items-center justify-center gap-3">
        <button
          ...
          className={`text-sm font-medium ${!annual ? "text-ink" : "text-ink-4"}`}
        >
          Monthly
        </button>
        <button
      ...
      <div className="mt-10 grid gap-6 sm:grid-cols-2">
          ...
          <ul className="mt-6 space-y-3">
            {FREE_FEATURES.map((f) => (
              <li key={f} className="flex items-center gap-2 text-sm text-ink-2">
                <Check className="h-4 w-4 text-ink-4" />
                {f}
              </li>
            ))}
          </ul>
          <div className="mt-8">
            <Show when="signed-out">
              <SignUpButton mode="modal" forceRedirectUrl="/onboarding">
                <button className="w-full rounded-lg border border-line-2 px-4 py-2 text-sm font-medium text-ink hover:bg-bg-2">
                  Start free
                </button>
              </SignUpButton>
            </Show>
            <Show when="signed-in">
              <Link
                href="/dashboard"
                className="block w-full rounded-lg border border-line-2 px-4 py-2 text-center text-sm font-medium text-ink hover:bg-bg-2"
              >
                Go to dashboard
              </Link>
            </Show>
        ...
        <div className="rounded-xl border-2 border-brand bg-paper p-8 shadow-sm">
          ...
          <div className="mt-8">
            <Show when="signed-out">
              <SignUpButton mode="modal" forceRedirectUrl="/onboarding">
                <button className="w-full rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white hover:opacity-90">
            ...
            <Show when="signed-in">
              <Link
                href="/billing"
                className="block w-full rounded-lg bg-brand px-4 py-2 text-center text-sm font-medium text-white hover:opacity-90"
              >
                Upgrade to Pro
              </Link>
            </Show>
